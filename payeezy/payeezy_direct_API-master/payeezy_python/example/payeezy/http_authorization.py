...
# support functions for HTTPS connections - dependancy for HTTPAdapter
import socket
import ssl
...
class PayeezyHTTPAuthorize(object):
    ...
    def generateHMACAuthenticationHeader(self, payload):
        ...
        hmacInHex = hmac.new(self.apisecret, msg=messageData,
    ...
    def getTokenPostCall(self, payload):
        ...
        response.veirfy = False
        response.mount('https://', MyAdapter())
        self.payload = json.dumps(payload)
        authorizationVal = self.generateHMACAuthenticationHeader(
            payload=self.payload)
        result = response.post(self.tokenURL, headers={'User-Agent': 'Payeezy-Python', 'content-type': 'application/json',
                                                       'apikey': self.apikey, 'token': self.token, 'Authorization': 'xxxxx'}, data=self.payload)
        return result
    ...
    def makeCardBasedTransactionPostCall(self, payload):
        ...
        result = response.post(self.url, headers={'User-Agent': 'Payeezy-Python', 'content-type': 'application/json', 'apikey': self.apikey,
                                                  'token': self.token, 'nonce': self.nonce, 'timestamp': self.timestamp, 'Authorization': authorizationVal}, data=self.payload)
